...
public class BusinessLayer{
   ...
   public Appointment newAppt = null;
   
   
public String addAppointment(@Context UriInfo c, String xmlStyle){
      String string = "";
      ...
      try{
         ...
         Document doc = db.parse(is);
          ...
          time = doc.getElementsByTagName("time").item(0).getTextContent() + ":00";
    ...
    for(int ii=0; ii <labTests.getLength(); ii++){     
      ...
      SimpleDateFormat tc1 = new SimpleDateFormat("HH:mm:ss");
      ...
      SimpleDateFormat tc2 = new SimpleDateFormat("HH:mm:ss");
      ...
      List<AppointmentLabTest> altList = null;
       ...
       if(dbSingleton.db.getData("Appointment","").contains(appt)){
            ...
            if(error.isEmpty() || error == null){  
                ...
                if(dbSingleton.db.getData("Patient", "").contains(patient)== true &&
                   dbSingleton.db.getData("Phlebotomist", "").contains(phleb)== true &&
                   dbSingleton.db.getData("PSC", "").contains(psc)== true &&
                   dbSingleton.db.getData("Physician", "").contains(phys) == true && 
        ...
        for (Object obj : objs){         
         ...
         string += "<AppointmentList><uri>"+c.getBaseUri().toString()+"Services/Appointments/"+apptId+"</uri></AppointmentList>"; 
       } else {
         string = "<?xml version='1.0' encoding='UTF-8' standalone='no'?><AppointmentList><error>ERROR:Appointment is not available</error></AppointmentList>";
